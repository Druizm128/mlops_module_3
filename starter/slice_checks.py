...
def check_slices_performance(data, model_path):
    ...
    for group in cat_features:
        performance=(test
            .assign(y_true=y_test, y_pred=predictions).groupby([group])
            .apply(lambda df: compute_model_metrics(df["y_true"], df["y_pred"]))
            .apply(pd.Series)
            .rename(columns={0:'precision', 1:'recall', 2:'fbeta'})
            .round(2)
            .reset_index()
            .assign(category=group)
            .rename(columns={group:'subcategory'})
            .filter(['category', 'subcategory', 'precision', 'recall', 'fbeta'])
        )
